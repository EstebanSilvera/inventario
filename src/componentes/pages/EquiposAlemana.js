...
const EquiposAlemana = () => {
  ...
  if (impcel.message === "no hay resultados") {
    ...
    impresora = impcel.filter((datos) => datos.id_sede === 4 && datos.Tipo_equipo === "Impresora" && datos.eliminado === 0)
    memoria = impcel.filter((datos) => datos.id_sede === 4 && datos.Tipo_equipo === "Memoria" && datos.eliminado === 0)
    celular = impcel.filter((datos) => datos.id_sede === 4 && datos.Tipo_equipo === "Celulares" && datos.eliminado === 0)
  }
  ...
  comercial = equipos.filter(datos => datos.id_sede === 4 && datos.eliminado === 0 && datos.grupo_trabajo === "COMERCIAL")
  almacen = equipos.filter(datos => datos.id_sede === 4 && datos.eliminado === 0 && datos.grupo_trabajo === "ALMACEN")
  taller = equipos.filter(datos => datos.id_sede === 4 && datos.eliminado === 0 && datos.grupo_trabajo === "TALLER")
  colision = equipos.filter(datos => datos.id_sede === 4 && datos.eliminado === 0 && datos.grupo_trabajo === "COLISION")
  administracion = equipos.filter(datos => datos.id_sede === 4 && datos.eliminado === 0 && datos.grupo_trabajo === "ADMINISTRACION")
  //

  let array_sede, buscar_sede, buscar_impresora, buscar_celular = []
  array_sede = equipos.filter(datos => datos.id_sede === 4 && datos.eliminado === 0)
  ...
  return (
    <div style={{ background: 'linear-gradient( #90A4AE, #A9CCE3 , #90A4AE)' }} ><br />
      ...
      <div className='container'>
        ...
        <h1 align='center'>Equipos de computo en Alemana </h1><br />
        <div align='center'>
          <Card>
...
function Eliminar(props) {
  ...
  const actualizar = async () => {
    ...
    if (mostrar === "1") {
      ...
      setArrayall(props.equipos.filter(dato => dato.eliminado === 0 && dato.id_sede === 4))
    } else if (mostrar === "2") {

      setArrayall(props.equiposimp.filter((datos) => datos.id_sede === 4 && datos.Tipo_equipo === "Impresora" && datos.eliminado === 0))
    } else if (mostrar === "3") {

      setArrayall(props.equiposimp.filter((datos) => datos.id_sede === 4 && datos.Tipo_equipo === "Celulares" && datos.eliminado === 0))
    } else if (mostrar === "4") {

      setArrayall(props.equiposimp.filter((datos) => datos.id_sede === 4 && datos.Tipo_equipo === "Memoria" && datos.eliminado === 0))
    }
...
function Actualizar(props) {
  ...
  let equipo_filter = []
  equipo_filter = props.equipos.filter(datos => datos.id_sede === 4 && datos.eliminado === 0)
...
function Eliminados(props) {
  ...
  let array_eliminado = []
  array_eliminado = props.equipos.filter(datos => datos.id_sede === 4 && datos.eliminado === 1)

  let array_imp, array_cel, memoria = []

  if (props.equiposimp.message === "no hay resultados") {
  } else {
    array_imp = props.equiposimp.filter((datos) => datos.id_sede === 4 && datos.Tipo_equipo === "Impresora" && datos.eliminado === 1)

    array_cel = props.equiposimp.filter((datos) => datos.id_sede === 4 && datos.Tipo_equipo === "Celulares" && datos.eliminado === 1)

    memoria = props.equiposimp.filter((datos) => datos.id_sede === 4 && datos.Tipo_equipo === "Memoria" && datos.eliminado === 1)
  }
...
function Pdf(props) {
  ...
  //   //console.log(responseJSON)
  // }
  let array_equipo = props.equipos.filter((datos) => datos.id_sede === 4 && datos.eliminado === 0)
